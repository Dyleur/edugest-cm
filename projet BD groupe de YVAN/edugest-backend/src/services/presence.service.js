...
const getPresencesByEleve = async (matricule) => {
  const presences = await Frequente.findAll({
    where: { matricule },
    include: [
      { model: Salle, attributes: ['idSalle', 'libelle'] },
      { model: AnneeAcademique, attributes: ['idAnnee', 'libelle'] }
    ],
    order: [['created_at', 'DESC']]
  });
  return presences.map(p => ({
    ...p.toJSON(),
    statut: p.commentaire ? p.commentaire.split('|')[0] : 'INCONNU',
    motif: p.commentaire ? p.commentaire.split('|')[1] || '' : ''
  }));
};
